...
describe("parseListeners", () => {
  it("reads pid, process, address and port from every family", () => {
    expect(parseListeners(LSOF)).toEqual([
      ...
      { pid: 49046, processName: "Code\\x20H", address: "127.0.0.1", port: 41729 },
...
describe("parseLabels", () => {
  it("reads label and scheme", () => {
    ...
    expect(parseLabels(content)).toEqual(
      new Map([
        [3000, { label: "Frontend", scheme: null }],
        [4443, { label: "API", scheme: "https" }],
      ]),
    );
  });

  it("labels nothing when the file is malformed", () => {
    expect(parseLabels("{not json")).toEqual(new Map());
    expect(parseLabels(JSON.stringify({ ports: "nope" }))).toEqual(new Map());
  });

  it("skips entries with a bad port, label or scheme", () => {
    const content = JSON.stringify({
      ports: [
        { port: "3000", label: "string port" },
        { port: 70000, label: "out of range" },
        { port: 3001, label: "  " },
        { port: 3002, label: "ok", scheme: "ftp" },
      ],
    });
    expect(parseLabels(content)).toEqual(
      new Map([[3002, { label: "ok", scheme: null }]]),
    );
  });
});
